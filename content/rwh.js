...
var ReplyWithHeader = {
  ...
  handleBadMimeDateUsers: function(rawHdr, pHeader) {
    ...
    var date;
    if (! luser)
      date = pHeader.mimeDate;
    else
    if (luser.tzOffset)
      date = this.fixupTimezone(pHeader.mimeDate, luser.tzOffset)
    else
      date = pHeader.recvDate;

    pHeader.date = this.formatMimeDate(date);
  },
  ...
  get createRwhHeader() {
    ...
    if (this.Prefs.useSenderDate) {
      pHeader.mimeDate = MimeHeaders(this.messageUri).get("Date");
      pHeader.recvDate = new Date(rawHdr.date/1000)
